...
### Headed Playwright QA Audit — Issue #273, #274, #275, #276 (2026-03-15)
- **Context:** User directive to run Playwright in HEADED mode with real GPU, observe Flora's actual visuals, and create quality backlog from player experience (not code reading).
- **Key Insight:** Headed WebGL works perfectly. SwiftShader headless mode was producing false positives — real GPU renders Flora instantly with zero timeout issues.
- **Test Infrastructure:** Configured playwright.config.ts for headed mode (headless: false, slowMo: 500, --enable-gpu). Fixed baseURL to localhost:3000/flora/ (was pointing to wrong project on :5173). Created comprehensive gameplay test (flora-gameplay.spec.ts) with screenshot capture at every step. Test ran 1.2 minutes, captured 14 screenshots, 0 console errors.
- **CRITICAL DISCOVERY (P0):** Menu keyboard navigation is BROKEN (#273). Pressing Enter on the main menu opens the Achievements screen instead of starting a new run. Keyboard-only players cannot start the game. This blocks ALL garden gameplay QA.
- **Visual Quality Findings (What I DID See):** Boot screen is GORGEOUS — warm forest green palette (#2d5016), animated particles (yellow/orange/sage dots), clean FLORA logo with leaf icon, "A cozy gardening roguelite" tagline. Main menu has excellent button layout, clear visual focus state (green border), helpful keyboard hints. Achievements screen well-organized with category tabs (Harvest/Survival/Synergy/Exploration/Mastery), locked state UX clear (padlock icon + "???"). Typography is clean throughout, contrast excellent (WCAG AAA on most elements).
- **Issues Created:** #273 (P0) — Menu Enter key opens Achievements instead of New Run (blocks gameplay). #274 (P2) — Boot "Press any key" hint has low contrast, easy to miss. #275 (P1) — Locked achievements have no hover feedback when clicked (player confusion). #276 (P2) — Menu buttons lack hover states (reduced tactile confidence).
- **What I COULDN'T See (Blocked by #273):** Garden screen, seed selection, planting interactions, watering visual feedback, plant growth stages, day transitions, results screen. Cannot perform visual QA on core 90% of gameplay until navigation bug is fixed.
- **Positive Findings:** Cozy aesthetic is REAL and screenshot-worthy. Warm earth tones throughout. Visual continuity from boot → menu → achievements. Zero inline color values (all use config constants ✅). Particle effects add life without clutter. Icon usage aids navigation. Zero JS console errors during entire test session.
- **Architectural Recommendation:** MenuScene keyboard handler has focus state mismatch — visual highlight (green border) != actual keyboard focus index. Enter key likely calls wrong menu option. Add debug logging to verify selectedIndex maps to correct button.
- **Testing Paradigm Established:** Screenshot-based QA from player perspective. Run headed test → Analyze visuals → File issues based on "Would I show this to a friend?" Not "Does code follow conventions?" but "Does THIS SCREEN feel cozy?"
- **Next Steps:** Fix #273 (assign to Misty - UI specialist). Re-run flora-gameplay.spec.ts to capture garden screenshots. Create Round 2 backlog from actual garden visuals. Integrate headed tests into CI with Xvfx (Linux) or Docker (Windows).
- **Branch:** squad/oak-headed-playwright-qa (e83c5ba). Files: playwright.config.ts (headed config), tests/e2e/flora-gameplay.spec.ts (NEW comprehensive gameplay test), .squad/decisions/inbox/oak-headed-qa.md (full findings report with screenshot analysis).
- **Strategic Impact:** We now have EYES. Headed mode shows the REAL game. Flora's UI layer is beautiful. But P0 navigation bug must be fixed before we can validate the garden — where players spend 90% of their time.
